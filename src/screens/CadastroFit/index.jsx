...
const CadastroFit = ({ setAuthenticated, user }) => {
  ...
  const { isLoading: isLoadingRequest, data } = useGet({
    ...
    mutate(
      {
        ...
        weight: weight,
        age: age,
        fitness_level: exerciseRegularity,
        gender: gender,
      },
      {
        onSuccess: (data) => {
        ...
        <form onSubmit={submitHandler}>
          ...
          <div className="input-box col-12 mb-3">
            ...
            >
              ...
              <option value="Avançado">Professor Jhonny</option>
            </select>
          </div>
          ...
          >
            Salvar{" "}
            {isLoadingRequest && (
              <img alt="" src="/assets/loading.svg" width={24} />
            )}
          </button>
      ...
      <div className="content-init">
        ...
        <h5 className="f14 green fw-700 fw-bold text-start">
          {isLoadingData
            ...
            : "Cadastre-se para obter os treinos"}
        </h5>
      </div>
    </>
  );
